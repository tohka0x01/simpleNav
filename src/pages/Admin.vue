...
async function removeSite(s){
  ...
  try{
    const res = await fetch('/api/sites/delete', { method:'POST', headers: authHeaders({ 'content-type':'application/json' }), body: JSON.stringify({ id: s.id }) });
...
async function saveCategory(){
  ...
  try{
    const res = await fetch('/api/categories/add', { method:'POST', headers: authHeaders({ 'content-type':'application/json' }), body: JSON.stringify({ name: categoryName.value, desc: categoryDesc.value }) });
    if(res.ok) invalidateCache();
    await loadCategories();
...
async function applyEdit(){
  ...
  try{
    const res = await fetch('/api/categories/update', { method:'POST', headers: authHeaders({ 'content-type':'application/json' }), body: JSON.stringify({ name: editingName.value, newName: editName.value, desc: editDesc.value }) });
    if(res.ok) invalidateCache();
    await loadCategories();
  }finally{ cancelEdit(); }
}
async function removeCategory(name:string){
  ...
  try{
    const res = await fetch('/api/categories/delete', { method:'POST', headers: authHeaders({ 'content-type':'application/json' }), body: JSON.stringify({ name }) });
    if(res.ok) invalidateCache();
    await loadCategories();
...
async function submit(){
  ...
  try{
    ...
    if(res.ok){ okMsg.value='已添加'; resetForm(); invalidateCache(); await load(); closeAdd(); }
